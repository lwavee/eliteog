...
export const TestimonialCarousel: React.FC<TestimonialCarouselProps> = ({ testimonials }) => {
  ...
  return (
    <div className="relative max-w-4xl mx-auto flex flex-col items-center py-10 px-6">
      ...
      <div className="absolute top-0 left-10 text-royal-100 pointer-events-none">
      ...
      <div className="relative w-full overflow-hidden min-h-[300px] flex items-center justify-center">
        <AnimatePresence mode="wait">
          ...
          >
            ...
            {/* Review content */}
            <blockquote className="text-lg md:text-xl lg:text-2xl text-slate-700 italic max-w-2xl font-light leading-relaxed">
              "{current.review}"
            </blockquote>
            ...
            <div className="flex flex-col items-center gap-1.5 mt-4">
              {current.avatar ? (
                <img
                  src={current.avatar}
                  alt={current.author}
                  className="w-14 h-14 rounded-full object-cover border border-royal-200"
                />
              ) : (
              ...
              <h5 className="font-bold text-navy-950 text-base mt-2 font-display">{current.author}</h5>
              <p className="text-xs text-slate-500">
                {current.position}, <span className="text-royal-600 font-semibold">{current.company}</span>
              </p>
            </div>
          </motion.div>
        </AnimatePresence>
      ...
      <div className="flex gap-4 mt-8 z-10 relative">
        <button
          onClick={prevSlide}
          className="p-3.5 rounded-full bg-white hover:bg-slate-50 border border-slate-200 text-royal-600 transition-all cursor-pointer shadow-sm focus:outline-none"
        >
